...
class I2SClocklessLedDriver
{
  ...
  const char *TAG_I2S = "I2SClocklessLedDriver";
  ...
  void showPixelsFromBuffer(displayMode dispmode)
  {
    /*
       ...
       */
    if (__displayMode == CONT && isDisplaying)
    {
      ESP_LOGE(TAG_I2S, "The loop mode is activated execute stopDisplayLoop() first");
      return;
    }
    /*
    ...
    isWaiting = false;
    if (dispmode == CONT or dispmode == LOOP_INTERUPT)
    {
...
*/
  ...
  void __showPixels()
  {
    if(!__enableDriver)
    {
      ESP_LOGE("__showPixels","Driver is disabled");
      return;
    }
#ifdef __HARDWARE_MAP
    _hmapoff=_hmap;
    ...
    if (leds == NULL)
    {
      ESP_LOGE(TAG_I2S, "no leds buffer defined");
      return;
    }
    ledToDisplay = 0;
    transpose = true;
  ...
  void initled(uint8_t *leds, int *Pinsq, int *sizes,  int num_strips)
  {
    ...
    ESP_LOGV(TAG_I2S, "maximum leds%d\n",maximum);
  ...
  void createhardwareMap()
  {
    ...
    ESP_LOGE(TAG_I2S,"trying to map2");
    int offset2=0;
  ...
  void __initled(uint8_t *leds, int *Pinsq, int num_strips, int num_led_per_strip)
  {
    ...
    ESP_LOGV(TAG_I2S,"xdelay:%d",__delay);
#if HARDWARESPRITES == 1
    ...
    if(!_hmap)
    {
      ESP_LOGE(TAG_I2S,"no memory for the hamp");
    }
    else
    {
      ESP_LOGE(TAG_I2S,"trying to map");
      /*
         for(int leddisp=0;leddisp<num_led_per_strip;leddisp++)
         {
         for (int i = 0; i < num_strips; i++)
  ...
  I2SClocklessLedDriverDMABuffer *allocateDMABuffer(int bytes)
  {
    ...
    if (!b)
    {
      ESP_LOGE(TAG_I2S, "No more memory\n");
      return NULL;
    }
